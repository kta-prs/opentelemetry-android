...
internal class ComposeClickSpanGenerator(
    private val tracer: ExtendedTracer,
    ...
    fun generateClick(motionEvent: MotionEvent?) {
        windowRef?.get()?.let { window ->
            if (motionEvent != null && motionEvent.actionMasked == MotionEvent.ACTION_UP) {
                val layoutNode: LayoutNode? = composeTapTargetDetector.findTapTarget(
                    window.decorView,
                    motionEvent.x,
                    motionEvent.y
                )

                val span = tracer.spanBuilder(APP_SCREEN_CLICK_SPAN_NAME)
                    .setAttribute(APP_SCREEN_COORDINATE_Y, motionEvent.y.toLong())
                    .setAttribute(APP_SCREEN_COORDINATE_X, motionEvent.x.toLong())
                    .startSpan()
                println("DEBUG-SPAN : Created span for click at (${motionEvent.x}, ${motionEvent.y})")
                layoutNode?.let {
                    span.setAllAttributes(createNodeAttributes(it))
                    println("DEBUG-SPAN : Click target - ${createNodeAttributes(it)}")
                }
                span.end()
